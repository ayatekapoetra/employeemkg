...
import { AppScreen, HeaderScreen, LoadingHauler, BottomSheetSelect } from '../../src/components/common';
...
/** Filter absensi bulanan hanya untuk role ini */
...
/** Normalisasi payload HRIS agar selaras dengan UI */
function normalizeMonthlyPayload(payload) {
  ...
  // merge detailsByDate dari server jika ada
...
function StatPill({ label, value, color, isDark }) {
  return (
    <VStack
      ...
      p={3}
      rounded="2xl"
      bg={isDark ? 'rgba(255,255,255,0.04)' : '#fff'}
      borderWidth={1}
    ...
    >
      <HStack alignItems="center" space={1.5}>
        ...
        <Text fontSize={11} fontFamily="Poppins-Regular" color={isDark ? '#94a3b8' : '#64748b'}>
          {label}
        </Text>
      </HStack>
      <Text fontSize={22} fontFamily="Quicksand-Bold" color={isDark ? '#F5F5F5' : '#0f172a'}>
        {value}
      </Text>
    </VStack>
  );
...
function LegendItem({ meta, isDark }) {
  return (
    <HStack alignItems="center" space={1.5} mr={3} mb={2}>
      <Box
        ...
        h={3}
        rounded="md"
        bg={isDark ? meta.bgDark : meta.bgLight}
        borderWidth={1}
...
function MonthCalendarGrid({ calendar, monthKey, isDark, onDayPress, t }) {
  const start = moment(monthKey, 'YYYY-MM').startOf('month');
  const startPad = start.day(); // 0=Sun
  ...
  for (let d = 1; d <= daysInMonth; d += 1) {
    ...
    const dayData = calendar.find(c => c.date === date) || { date, day: d, status: null };
    ...
    <Box bg={t.cardBg} rounded="2xl" p={3.5} borderWidth={1} borderColor={t.border} style={styles.cardShadow}>
      <HStack mb={3} flexWrap="wrap">
        {Object.values(STATUS_META).map(meta => (
          <LegendItem key={meta.key} meta={meta} isDark={isDark} />
        ))}
      </HStack>
      ...
      <Box flexDirection="row" flexWrap="wrap">
        {cells.map(cell => {
          if (cell.empty) {
            return <Box key={cell.key} width="14.285%" aspectRatio={1} p={0.5} />;
          }

          const { dayData } = cell;
          const meta = dayData.status ? STATUS_META[dayData.status] : null;
          const isToday = dayData.date === todayStr;
          // Background cell: status tint lebih tegas, default cell tetap kontras thd card
          const bg = meta
            ? isDark
              ? meta.bgDark
              : meta.bgLight
          ...
          const borderC = meta
            ...
            : isToday
              ...
              : isDark
                ...
                : 'rgba(15,23,42,0.08)';
          // Angka tanggal: kontras tinggi thd background cell + theme
              ...
              >
                ...
                >
                  <Text
                    ...
                    style={{ textShadowColor: isDark ? 'rgba(0,0,0,0.45)' : 'transparent', textShadowRadius: isDark ? 2 : 0 }}
                  >
                    {dayData.day}
                  </Text>
                  {meta ? (
...
function DetailModal({ visible, onClose, dayData, isDark, onPreviewPhoto }) {
  ...
  const dateLabel = dayData?.date
    ? moment(dayData.date).locale('id').format('dddd, DD MMMM YYYY')
    : '-';
  ...
  const viaLabel = via => {
    if (via === 'M') return 'Mesin';
    if (via === 'A') return 'Mobile';
    return '-';
  };
  ...
  const PhotoBlock = ({ title, time, photo, via, accent, Icon }) => (
    <VStack flex={1} space={2} alignItems="center">
      <HStack alignItems="center" space={1}>
        ...
        <Text fontSize={12} fontFamily="Quicksand-Bold" color={t.muted}>
          {title}
        </Text>
      </HStack>
      <TouchableOpacity
        activeOpacity={0.85}
        disabled={!photo}
        onPress={() => photo && onPreviewPhoto(photo)}
      >
        <Box
          w={28}
          h={28}
          ...
          overflow="hidden"
          borderWidth={2}
          borderColor={photo ? accent : t.border}
          bg={t.panelBg}
          alignItems="center"
        ...
        >
          {photo ? (
            ...
            <Profile size={28} color={isDark ? '#64748b' : '#94a3b8'} variant="Bulk" />
          )}
        </Box>
      </TouchableOpacity>
      <Text fontSize={22} fontFamily="Quicksand-Bold" color={t.text}>
        {time}
      </Text>
      <Box
        px={2.5}
      ...
      >
        <Text fontSize={10} fontFamily="Quicksand-SemiBold" color={t.muted}>
          {viaLabel(via)}
        </Text>
      </Box>
    </VStack>
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <Box flex={1} bg="rgba(0,0,0,0.55)" justifyContent="flex-end">
        <Box bg={t.cardBg} borderTopRadius={28} maxH="88%" pb={6}>
          <Center pt={3} pb={1}>
            <Box w={10} h={1} rounded="full" bg={isDark ? '#4b5563' : '#d1d5db'} />
          </Center>

          <HStack px={5} py={3} alignItems="center" justifyContent="space-between" borderBottomWidth={1} borderBottomColor={t.border}>
            <VStack flex={1} pr={3}>
              <Text fontSize={18} fontFamily="Quicksand-Bold" color={t.text}>
                Detail Kehadiran
              </Text>
              <Text fontSize={13} fontFamily="Poppins-Regular" color={t.muted}>
                {dateLabel}
              </Text>
            </VStack>
            {meta ? (
              <Box px={3} py={1} rounded="full" bg={isDark ? meta.bgDark : meta.bgLight}>
                <Text fontSize={12} fontFamily="Quicksand-Bold" color={meta.color}>
                  {meta.label}
                </Text>
              </Box>
            ) : null}
            <TouchableOpacity onPress={onClose} style={{ marginLeft: 10 }}>
              <Box
                ...
                justifyContent="center"
              >
                <CloseCircle size={18} color={t.muted} variant="Bold" />
              </Box>
            </TouchableOpacity>
          </HStack>
          ...
          <ScrollView contentContainerStyle={{ padding: 20 }} showsVerticalScrollIndicator={false}>
            {!dayData || (!dayData.status && !dayData.checklog_in) ? (
              <Center py={10}>
                <InfoCircle size={36} color={t.muted} variant="Bulk" />
                <Text mt={3} fontFamily="Poppins-Regular" color={t.muted}>
              ...
              <VStack space={4}>
                ...
                >
                  ...
                  >
                    <Profile size={24} color={t.accent} variant="Bulk" />
                  </Center>
                  <VStack flex={1}>
                    <Text fontSize={15} fontFamily="Quicksand-Bold" color={t.text}>
                      {dayData.karyawan?.nama || 'Karyawan'}
                    </Text>
                    <Text fontSize={12} fontFamily="Poppins-Regular" color={t.muted}>
                      PIN {dayData.pin || '-'} · Shift {dayData.shift || '-'}
                    </Text>
                  </VStack>
                </HStack>

                {(dayData.checklog_in || dayData.checklog_out || ['hadir', 'terlambat'].includes(dayData.status)) && (
                  <Box p={4} rounded="2xl" bg={t.panelBg} borderWidth={1} borderColor={t.border}>
                    <HStack alignItems="stretch" space={3}>
                      <PhotoBlock
                        title="MASUK"
                        time={fmt(dayData.checklog_in)}
                        photo={dayData.photo_in}
                        via={dayData.via_in}
                        accent={isDark ? '#34d399' : '#059669'}
                        Icon={LoginCurve}
                      />
                      <Box w={0.5} bg={t.border} alignSelf="stretch" />
                      <PhotoBlock
                        title="PULANG"
                        time={fmt(dayData.checklog_out)}
                        photo={dayData.photo_out}
                        via={dayData.via_out}
                        accent={isDark ? '#60a5fa' : '#2563eb'}
                        Icon={LogoutCurve}
                      />
                ...
                <VStack space={2.5}>
                  {duration ? (
                    <HStack
                      ...
                      space={3}
                    >
                      <Center w={10} h={10} rounded="xl" bg={isDark ? 'rgba(99,102,241,0.2)' : 'rgba(99,102,241,0.12)'}>
                        <Clock size={20} color="#6366f1" variant="Bulk" />
                      </Center>
                      <VStack>
                        <Text fontSize={11} fontFamily="Poppins-Regular" color={t.muted}>
                          Durasi kerja
                        </Text>
                        <Text fontSize={15} fontFamily="Quicksand-Bold" color={t.text}>
                          {duration}
                  ...
                  {dayData.terlambat === 'Y' ? (
                    <HStack
                      p={3.5}
                      rounded="2xl"
                      bg={isDark ? STATUS_META.terlambat.bgDark : STATUS_META.terlambat.bgLight}
                      borderWidth={1}
                      borderColor={STATUS_META.terlambat.color}
                      alignItems="center"
                      space={3}
                    >
                      <Warning2 size={22} color={STATUS_META.terlambat.color} variant="Bulk" />
                      <VStack>
                        <Text fontSize={11} fontFamily="Poppins-Regular" color={STATUS_META.terlambat.color}>
                          Keterlambatan
                        </Text>
                        <Text fontSize={15} fontFamily="Quicksand-Bold" color={STATUS_META.terlambat.color}>
                          {dayData.durasi_keterlambatan || 0} menit
                        </Text>
                      </VStack>
                    </HStack>
                  ) : null}
                  ...
                  {dayData.lokasi ? (
                    <HStack
                      ...
                      space={3}
                    >
                      <Center w={10} h={10} rounded="xl" bg={isDark ? 'rgba(245,158,11,0.2)' : 'rgba(245,158,11,0.12)'}>
                        <Location size={20} color="#f59e0b" variant="Bulk" />
                      </Center>
                      <VStack flex={1}>
                        <Text fontSize={11} fontFamily="Poppins-Regular" color={t.muted}>
                          Lokasi
                        </Text>
                        <Text fontSize={14} fontFamily="Quicksand-Bold" color={t.text}>
                          {dayData.lokasi}
                        </Text>
                      </VStack>
                    </HStack>
                  ...
                  {dayData.note ? (
                    <HStack
                      ...
                      space={3}
                    >
                      <Center w={10} h={10} rounded="xl" bg={isDark ? 'rgba(148,163,184,0.2)' : 'rgba(148,163,184,0.15)'}>
                        {dayData.status === 'sakit' ? (
                          <Hospital size={20} color="#ec4899" variant="Bulk" />
                        ) : (
                          <InfoCircle size={20} color={t.muted} variant="Bulk" />
                        )}
                      </Center>
                      <VStack flex={1}>
                        <Text fontSize={11} fontFamily="Poppins-Regular" color={t.muted}>
                        ...
                        <Text fontSize={13} fontFamily="Poppins-Regular" color={t.text}>
                          {dayData.note}
                        </Text>
                      </VStack>
                    </HStack>
    ...
    <Modal visible={visible} transparent animationType="none" onRequestClose={closeSheet}>
      <Box flex={1} justifyContent="flex-end">
        ...
        >
          ...
          <HStack px={4} py={3} alignItems="center" justifyContent="space-between" borderBottomWidth={1} borderBottomColor={t.border}>
            <HStack alignItems="center" space={2}>
              <FilterSearch size={20} color={t.accent} variant="Bold" />
              <VStack>
                <Text fontSize={16} fontFamily="Quicksand-Bold" color={t.text}>
                  Filter Kehadiran
                </Text>
                <Text fontSize={11} fontFamily="Poppins-Regular" color={t.muted}>
                  Bisnis · Site · Karyawan
                </Text>
              </VStack>
            </HStack>
            <TouchableOpacity onPress={closeSheet} hitSlop={10}>
              <CloseCircle size={22} color={t.muted} variant="Bold" />
            </TouchableOpacity>
          </HStack>
          ...
          >
            <VStack space={4}>
              <BottomSheetSelect
                label="Bisnis Unit"
                placeholder="Semua bisnis unit"
                value={draft.bisnis_id}
              ...
              <BottomSheetSelect
                ...
                value={draft.karyawan_id}
                options={karyawanOptions}
                onChange={id => setDraft(prev => ({ ...prev, karyawan_id: id }))}
                displayKey="nama"
                displaySubKey="subtitle"
              />

              <HStack space={3} mt={2}>
                <TouchableOpacity style={{ flex: 1 }} activeOpacity={0.85} onPress={onReset}>
                  <Center
                    py={3.5}
                    rounded="xl"
                    borderWidth={1}
                    borderColor={t.border}
                    bg={isDark ? 'rgba(255,255,255,0.04)' : 'rgba(15,23,42,0.04)'}
                  >
                    <Text fontFamily="Quicksand-Bold" color={t.muted}>
                      Reset
                    </Text>
                  </Center>
                </TouchableOpacity>
                ...
                >
                  <Center py={3.5} rounded="xl" bg={t.accent}>
                    <Text fontFamily="Quicksand-Bold" color="#fff">
                      Terapkan
                    </Text>
                  </Center>
                </TouchableOpacity>
...
export default function AttendanceHistoryScreen() {
  ...
  const cabangList = useSelector(state => state.cabang?.data) || [];
  ...
  // Load master data untuk filter (hanya role yang diizinkan)
  useEffect(() => {
    ...
    if (!Array.isArray(cabangList) || cabangList.length === 0) {
      dispatch(getCabang());
    }
  ...
  const bisnisOptions = useMemo(() => {
    return (bisnisList || [])
      .map(item => {
        ...
        return {
          id,
          nama: item.nama || item.name || item.initial || `Bisnis ${id}`,
          subtitle: item.initial || item.kode || '',
        };
      })
      .filter(Boolean);
  }, [bisnisList]);
  ...
  const siteOptions = useMemo(() => {
    ...
    return list
      .map(item => {
        ...
        if (!id) return null;
        const bisnisName =
          item.bisnis?.nama || item.bisnis?.name || item.bisnis_unit?.nama || item.nama_bisnis || '';
        return {
          id,
          nama: item.nama || item.name || `Site ${id}`,
          subtitle: [item.area || '', bisnisName].filter(Boolean).join(' · '),
          bisnis_id: item.bisnis_id ?? item.bisnis?.id ?? item.bisnis_unit_id ?? item.bisnis_unit?.id ?? null,
        };
      })
      .filter(Boolean);
  ...
  const karyawanOptions = useMemo(() => {
    ...
    if (activeSite) {
      rows = rows.filter(k => {
        const kidSite =
          k.cabang_id ?? k.site_id ?? k.cabang?.id ?? k.site?.id ?? null;
        return kidSite != null && String(kidSite) === String(activeSite);
      });
    } else if (activeBisnis) {
      const siteIds = (Array.isArray(cabangList) ? cabangList : [])
        .filter(c => {
          const cBid = c.bisnis_id ?? c.bisnis?.id ?? c.bisnis_unit_id ?? c.bisnis_unit?.id;
    ...
    return rows
      .map(item => {
        ...
        return {
          id,
          nama: item.nama || item.name || `Karyawan ${id}`,
          subtitle: [item.pin ? `PIN ${item.pin}` : '', item.nik || '', item.section || '']
            .filter(Boolean)
            .join(' · '),
          pin: item.pin,
          raw: item,
  ...
  const selectedFilterKaryawan = useMemo(() => {
    ...
    const found = (Array.isArray(karyawanList) ? karyawanList : []).find(
      k => String(k.id) === String(filter.karyawan_id)
    );
  ...
  const handleApplyFilter = next => {
    setFilter({
      bisnis_id: next.bisnis_id || null,
      site_id: next.site_id || null,
      karyawan_id: next.karyawan_id || null,
    });
  };
  ...
  const filterChipLabels = useMemo(() => {
    ...
    if (filter.karyawan_id) {
      chips.push({
        key: 'karyawan_id',
        label: selectedFilterKaryawan?.nama || `Karyawan #${filter.karyawan_id}`,
      });
    }
    return chips;
  ...
  const fetchMonthlyHris = useCallback(async () => {
    ...
    try {
      ...
      if (resp.data?.diagnostic?.error) {
        throw new Error(resp.data.diagnostic.message || 'Gagal memuat data kehadiran');
      }

      const payload = normalizeMonthlyPayload(resp.data?.data || resp.data);
      if (!payload?.calendar || !payload?.summary) {
        throw new Error('Format data kehadiran bulanan tidak valid');
      }
      ...
      setMonthData(null);
      setFetchError(
        err?.response?.data?.diagnostic?.message ||
          err?.response?.data?.message ||
          err?.message ||
          'Gagal memuat data kehadiran bulanan'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handlePreviousMonth = () => {
    setSelectedMonth(moment(selectedMonth, 'YYYY-MM').subtract(1, 'month').format('YYYY-MM'));
  };

  const handleNextMonth = () => {
    const next = moment(selectedMonth, 'YYYY-MM').add(1, 'month');
    if (next.isSameOrBefore(moment(), 'month')) {
  ...
  const handleDayPress = dayData => {
    ...
    if (detail) {
      setSelectedDay({
        ...detail,
        photo_in: resolvePhotoUri(detail.photo_in),
        photo_out: resolvePhotoUri(detail.photo_out),
      });
    } else {
      setSelectedDay(null);
    }
    ...
    <AppScreen>
      <VStack h="full" bg={t.pageBg}>
        <HeaderScreen
          ...
          onThemes
          onNotification
          onFilter={canUseFilter ? openFilterSheet : undefined}
        />
        ...
        <ScrollView
          ...
          refreshControl={
            <RefreshControl refreshing={loading} onRefresh={fetchMonthlyHris} tintColor={t.text} />
          }
          showsVerticalScrollIndicator={false}
        >
          {/* Active filter chips (role-gated) */}
          {canUseFilter ? (
            <HStack mb={3} alignItems="center" flexWrap="wrap" style={{ gap: 8 }}>
              <TouchableOpacity onPress={openFilterSheet} activeOpacity={0.85}>
                <HStack
                  ...
                  alignItems="center"
                  space={1.5}
                  bg={activeFilterCount > 0 ? (isDark ? 'rgba(96,165,250,0.2)' : 'rgba(37,99,235,0.12)') : t.cardBg}
                  borderWidth={1}
                  borderColor={activeFilterCount > 0 ? t.accent : t.border}
                >
                  <FilterSearch size={14} color={activeFilterCount > 0 ? t.accent : t.muted} variant="Bold" />
                  <Text fontSize={11} fontFamily="Quicksand-Bold" color={activeFilterCount > 0 ? t.accent : t.muted}>
              ...
              {filterChipLabels.map(chip => (
                <Box
                  ...
                  borderColor={t.border}
                >
                  <Text fontSize={11} fontFamily="Poppins-Regular" color={t.text} numberOfLines={1}>
                    {chip.label}
                  </Text>
                </Box>
              ))}
              ...
              {activeFilterCount > 0 ? (
                <TouchableOpacity onPress={handleResetFilter} hitSlop={8}>
                  <Text fontSize={11} fontFamily="Quicksand-Bold" color={STATUS_META.absen.color}>
                    Hapus
                  </Text>
                </TouchableOpacity>
              ) : null}
          ...
          >
            ...
            >
              <Profile size={26} color={t.accent} variant="Bulk" />
            </Center>
            <VStack flex={1}>
              <Text fontSize={15} fontFamily="Quicksand-Bold" color={t.text} numberOfLines={1}>
                {displayKaryawan?.nama || 'Karyawan'}
              </Text>
              <Text fontSize={12} fontFamily="Poppins-Regular" color={t.muted}>
                PIN {displayKaryawan?.pin || '-'} · {monthData?.monthName || selectedMonth}
              </Text>
            </VStack>
            <VStack alignItems="flex-end">
              <Text fontSize={20} fontFamily="Quicksand-Bold" color={t.accent}>
                {s.persentaseKehadiran}%
              </Text>
              <Text fontSize={10} fontFamily="Poppins-Regular" color={t.muted}>
                kehadiran
              </Text>
            </VStack>
          </HStack>
          ...
          <HStack
            ...
            style={styles.cardShadow}
          >
            <TouchableOpacity onPress={handlePreviousMonth} hitSlop={12} style={styles.navBtn}>
              <ArrowLeft2 size={22} color={t.text} variant="Bold" />
            </TouchableOpacity>
            <HStack alignItems="center" space={2}>
              <CalendarIcon size={18} color={t.accent} variant="Bulk" />
              <Text fontSize={16} fontFamily="Quicksand-Bold" color={t.text}>
            ...
            <TouchableOpacity
              ...
              disabled={moment(selectedMonth, 'YYYY-MM').isSame(moment(), 'month')}
              hitSlop={12}
              style={[
                styles.navBtn,
                moment(selectedMonth, 'YYYY-MM').isSame(moment(), 'month') && { opacity: 0.35 },
              ]}
            >
              <ArrowRight2 size={22} color={t.text} variant="Bold" />
            </TouchableOpacity>
          </HStack>
          ...
          >
            <HStack alignItems="center" justifyContent="space-between">
              <VStack space={1} flex={1} pr={3}>
                <Text fontSize={13} fontFamily="Quicksand-Bold" color={t.text}>
                  Ringkasan bulan ini
                </Text>
                <Text fontSize={12} fontFamily="Poppins-Regular" color={t.muted}>
                  {s.hadir + s.terlambat} hari hadir dari {s.totalHariKerja} hari kerja
                </Text>
                <HStack mt={2} space={3} flexWrap="wrap">
                  <HStack alignItems="center" space={1}>
                    <TickCircle size={14} color={STATUS_META.hadir.color} variant="Bold" />
                    <Text fontSize={11} color={t.muted} fontFamily="Poppins-Regular">
                      On-time {s.hadir}
                    </Text>
                  </HStack>
                  <HStack alignItems="center" space={1}>
                    <Warning2 size={14} color={STATUS_META.terlambat.color} variant="Bold" />
                    <Text fontSize={11} color={t.muted} fontFamily="Poppins-Regular">
                      Telat {s.terlambat}
                    </Text>
                  </HStack>
              ...
              >
                <Text fontSize={16} fontFamily="Quicksand-Bold" color={t.accent}>
                  {s.persentaseKehadiran}%
                </Text>
              </Center>
            </HStack>
          ...
          {loading ? (
            <Center py={12}>
              <LoadingHauler message="Memuat kehadiran bulanan..." type="default" />
            </Center>
          ) : fetchError ? (
            <Center py={12} px={4}>
              <InfoCircle size={36} color={STATUS_META.absen.color} variant="Bulk" />
              <Text mt={3} fontFamily="Quicksand-Bold" color={t.text} textAlign="center">
                Gagal memuat data
              </Text>
              <Text mt={1} fontSize={12} fontFamily="Poppins-Regular" color={t.muted} textAlign="center">
                {fetchError}
              </Text>
              <TouchableOpacity onPress={fetchMonthlyHris} style={{ marginTop: 16 }}>
                <Box px={5} py={2.5} rounded="xl" bg={t.accent}>
                  <Text color="#fff" fontFamily="Quicksand-Bold">
                    Coba lagi
                  </Text>
                </Box>
              </TouchableOpacity>
            ...
            <Center py={12}>
              <Text fontFamily="Poppins-Regular" color={t.muted}>
        ...
        <ImageViewing
          ...
          HeaderComponent={({ imageIndex }) => (
            <HStack px={4} pt={12} pb={2} alignItems="center" justifyContent="space-between" bg="rgba(0,0,0,0.4)">
              <VStack>
                <Text color="#fff" fontSize={14} fontFamily="Quicksand-Bold">
                  {gallery[imageIndex]?.label || 'Foto'}
                </Text>
                <Text color="rgba(255,255,255,0.7)" fontSize={11} fontFamily="Poppins-Regular">
                  {selectedDay?.karyawan?.nama || ''}
                </Text>
              </VStack>
          ...
          FooterComponent={({ imageIndex }) => (
            <HStack position="absolute" bottom={10} alignSelf="center" bg="rgba(0,0,0,0.55)" px={4} py={1.5} rounded="full">
