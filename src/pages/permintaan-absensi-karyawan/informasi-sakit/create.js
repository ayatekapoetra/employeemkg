...
import { Image, ArrowRight2, Calendar1, Camera, MessageText1, User, Like1, Calculator, UserOctagon } from 'iconsax-react-native'
...
const CreateFormSakit = () => {
    ...
    useMemo(() => {
        dispatch(getDataFetch({uri: `mobile-employee-approval-permission-diketahui/array`, params: {}}))
        setDiketahui(data)
    }, [])
    ...
    const onPostDataHandle = async () => {
        ...
        try {
            const resp = await apiFetch.post("hrd/permohonan-izin-sakit", state)
    ...
    return (
        <AppScreen>
            <VStack h={"full"}>
                <HeaderScreen title={"Form Informasi Sakit"} onBack={true} onThemes={true} onNotification={true}/>
                <VStack px={3} flex={1}>
                    ...
                    </TouchableOpacity>
                    {
                        ...
                        <PresenceTransition 
                            ...
                            <KaryawanList state={state} setState={setState} setOpenKaryawan={setOpenKaryawan}/>
                        </PresenceTransition>
                    }

                    <TouchableOpacity onPress={handleOpenSupervisor}>
                        <VStack mt={1}>
                            <HStack 
                                p={2} 
                                mt={1} 
                                ...
                                borderColor={appcolor.line[mode][1]}>
                                <HStack space={2} alignItems={"center"}>
                                    <UserOctagon size="32" color={appcolor.ico[mode][2]} variant="Bulk"/>
                                    <VStack>
                                        <Text 
                                            fontWeight={"bold"}
                                            fontFamily={"Poppins-Regular"}
                    ...
                    </TouchableOpacity>
                    {
                        ...
                        <PresenceTransition 
                            ...
                            initial={{opacity: 0, scale: 0 }}
                            animate={{ opacity: 1, scale: 1, transition: { duration: 250 } }}>
                            <ValidatorList field={['approved_by', 'approve']} data={data} state={state} setState={setState} setOpenValidator={setOpenSupervisor}/>
                        </PresenceTransition>
                    }
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                                    ...
                                    <HStack space={2} alignItems={"center"}>
                                        ...
                                        <VStack>
                                            ...
                                            <Text 
                                                ...
                                                fontFamily={"Quicksand-Regular"}
                                                color={appcolor.teks[mode][1]}>
                                                { moment(state.date_start).format('dddd, DD MMMM YYYY') }
                                            </Text>
                                        </VStack>
                                    </HStack>
                                    <ArrowRight2 size="16" color={appcolor.ico[mode][1]} variant="TwoTone"/>
                        ...
                        <DatePicker
                            ...
                            onConfirm={(date) => {
                                ...
                                var finish = moment(state.date_end)
                                setState({...state, date_start: date, jumlah: finish.diff(start, 'day')})
                            }}
                            onCancel={() => {
                                    ...
                                    <HStack space={2} alignItems={"center"}>
                                        ...
                                        <VStack>
                                            ...
                                            <Text 
                                                ...
                                                fontFamily={"Quicksand-Regular"}
                                                color={appcolor.teks[mode][1]}>
                                                { moment(state.date_end).format('dddd, DD MMMM YYYY') }
                                            </Text>
                                        </VStack>
                                    </HStack>
                                    <ArrowRight2 size="16" color={appcolor.ico[mode][1]} variant="TwoTone"/>
                        ...
                        <DatePicker
                            ...
                            date={new Date()}
                            theme={mode != "dark"?"light":"dark"}
                            minimumDate={new Date(moment(state.date_start).format("YYYY-MM-DD"))}
                            onConfirm={(date) => {
                                var start = moment(state.date_start)
                                var finish = moment(date)
                                setState({...state, date_end: date, jumlah: finish.diff(start, 'day')})
                            }}
                            onCancel={() => {
                        ...
                        <Modal.Content>
                            ...
                            <Modal.Body>
                                ...
                                <Input 
                                    ...
                                    onChangeText={(teks) => setState({...state, narasi: teks})}/>
                            </Modal.Body>
                            <Modal.Footer>
...
export default CreateFormSakit