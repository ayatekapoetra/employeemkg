...
import { TouchableOpacity, ActivityIndicator, RefreshControl, Dimensions } from 'react-native';
import { useSelector, useDispatch } from 'react-redux';
import { AppScreen, HeaderScreen, LoadingHauler } from '../../../src/components/common';
import CustomAlert from '../../../src/components/common/CustomAlert';
import { COLORS } from '../../../src/constants/colors';
import moment from 'moment';
import { Watch, Coffee, DocumentText, User, Timer, Calendar, Warning2, TickCircle, Danger as DangerIcon, Trash } from 'iconsax-react-native';
...
export default function CrewWorksheetShowScreen() {
  ...
  const handleUpdate = () => {
    ...
    router.push({
      pathname: '/operational/crew-worksheet/edit',
  ...
  const handleDelete = async () => {
    ...
    setConfirmAlert({
      ...
      message: 'Apakah Anda yakin ingin menghapus worksheet ini? Tindakan ini tidak dapat dibatalkan.',
      onConfirm: async () => {
        try {
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <Box
        ...
        py={1}
        rounded="full"
        style={{ shadowColor: statusColor, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.2, shadowRadius: 2, elevation: 2 }}>
  ...
  if (loading && !worksheet) {
    return (
      <AppScreen>
        <HeaderScreen title="Detail Worksheet" onBack={() => router.back()} onThemes={true} onNotification={true} />
        <LoadingHauler/>
      </AppScreen>
    );
  }
