...
export default function useEditItem(params, userProfile) {
  ...
  const handleSubmit = async (router) => {
    ...
    const allowedUserTypes = ['procurement', 'procurment', 'developer', 'pjo', 'direktur', 'administrator', 'keuangan'];
    ...
    if (!allowedUserTypes.includes(usertype)) {
      setValidationModal({
        ...
        type: 'error',
        title: 'Akses Ditolak',
        message: `Hanya user dengan role tertentu yang dapat melakukan edit.\n\nUser type Anda: ${userProfile?.usertype || 'tidak diketahui'}\nDibutuhkan: ${allowedUserTypes.join(', ')}`,
    ...
    const confirmMessage = `Apakah Anda yakin ingin menyimpan perubahan data item ini?\n\nRingkasan:\n• Qty: ${qtyAcc} unit\n• Harga: ${currency === 'USD' ? '$' : 'Rp'} ${harga.toLocaleString('id-ID')}\n• Total: Rp ${subtotal.toLocaleString('id-ID')}\n\nℹ️ Status approval tidak akan berubah.`;
    ...
    setValidationModal({
      ...
      onConfirm: async () => {
        try {
          ...
          const response = await apiClient.put(API_ENDPOINTS.PURCHASE_REQUEST.UPDATE_ITEM, payload);

          if (response.data?.diagnostic?.error === false) {
            const successMessage = `✅ Item berhasil diupdate!\n\nDetail:\n• Qty: ${qtyAcc} unit\n• Subtotal: ${currency === 'USD' ? '$' : 'Rp'} ${subtotal.toLocaleString('id-ID')}\n\nKembali ke halaman detail?`;
            ...
            setValidationModal({
              ...
              type: 'error',
              title: 'Update Gagal',
              message: response.data?.diagnostic?.message || 'Terjadi kesalahan saat mengupdate item.',
              onConfirm: () => setValidationModal(prev => ({ ...prev, visible: false })),
            });
          }
        } catch (error) {
          ...
          let errorMessage = 'Terjadi kesalahan saat mengupdate item.';
