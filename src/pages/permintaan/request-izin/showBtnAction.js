...
const ShowBtnAction = ( { data, user } ) => {
    ...
    const onApprovedHandle = async () => {
        console.log(`hrd/permohonan-izin-alpha/${data.id}/approval`);
        try {
            const resp = await apiFetch.post(`hrd/permohonan-izin-alpha/${data.id}/approval`, data)
    ...
    const onUpdateHandle = async () => {
        try {
            const resp = await apiFetch.post(`hrd/permohonan-izin-alpha/${data.id}/update`, data)
            if(resp.data.diagnostic.error){
                dispatch(applyAlert({
                    show: true,
                    status: "error",
    ...
    const onDestroyHandle = async () => {
        try {
            const resp = await apiFetch.post(`hrd/permohonan-izin-alpha/${data.id}/destroy`)
            if(resp.data.diagnostic.error){
                dispatch(applyAlert({
                    show: true,
                    status: "error",
    ...
    if(data.karyawan_id === user.karyawan.id && data.status === ""){
        return (
            <HStack space={1} mt={2}>
    ...
    }else if(data.karyawan_id === user.karyawan.id && data.status === "A"){
        return (
            <HStack space={1} mt={2}>
    ...
    }else if(["developer", "administrator", "hrd"].includes(user.usertype) && data.status === ""){
        return (
            <HStack space={1} mt={2}>
                <Button onPress={onDestroyHandle} w={"25%"} bg={appcolor.teks[mode][5]}>
                    <HStack space={1} alignItems={"center"}>
    ...
    }else if(["developer", "administrator", "hrd"].includes(user.usertype) && data.status === "A"){
        return (
            <HStack space={1} mt={2}>
