...
const ShowBtnAction = ( { data, user } ) => {
    ...
    const onApprovedHandle = async () => {
        try {
            const resp = await apiFetch.post(`hrd/permohonan-izin-cuti/${data.id}/approval`, data)
    ...
    const onUpdateHandle = async () => {
        try {
            const resp = await apiFetch.post(`hrd/permohonan-izin-cuti/${data.id}/update`, data)
            if(resp.data.diagnostic.error){
                dispatch(applyAlert({
                    show: true,
                    status: "error",
    ...
    const onDestroyHandle = async () => {
        try {
            const resp = await apiFetch.post(`hrd/permohonan-izin-cuti/${data.id}/destroy`)
            if(resp.data.diagnostic.error){
                dispatch(applyAlert({
                    show: true,
                    status: "error",
