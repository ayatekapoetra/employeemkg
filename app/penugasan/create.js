...
import DetailPenugasanCard from './components/DetailPenugasanCard';
...
import { clearCreateError } from '../../src/store/slices/equipmentPlanSlice';
import { 
  bulkCreateEquipmentPlan, 
  addDraftAssignment, 
  removeDraftAssignment,
  clearDraftAssignments,
  clearBulkCreateError
} from '../../src/store/slices/equipmentDraftPlanSlice';
...
export default function CreateEquipmentPlan() {
  ...
  const { draftAssignments, bulkCreateLoading, bulkCreateError } = useSelector(state => state.equipmentDraftPlan);
  ...
  useEffect(() => {
    if (bulkCreateError) {
      ...
      dispatch(clearBulkCreateError());
    }
  }, [bulkCreateError]);

  useEffect(() => {
    console.log('📊 Draft Assignment List State Changed (Redux):', {
      total: draftAssignments.length,
      isEmpty: draftAssignments.length === 0,
      summary: {
        total: draftAssignments.length,
        DT: draftAssignments.filter(a => a.equipmentKategori === 'DT').length,
        HE: draftAssignments.filter(a => a.equipmentKategori === 'HE').length,
      },
      assignments: draftAssignments.map((a, idx) => ({
        no: idx + 1,
        id: a.id,
        karyawan: a.labels.karyawan,
        equipment: a.labels.equipment,
        kategori: a.equipmentKategori,
      }))
    });
  }, [draftAssignments]);
  ...
  const validateForm = () => {
    ...
    if (!formData.karyawan_id) {
      setAlertConfig({
        ...
        type: 'warning',
        title: 'Validasi',
        message: 'Operator/Driver wajib dipilih',
        buttons: [{ text: 'OK', onPress: () => {} }]
      });
      return false;
    }
    if (!formData.lokasi_id) {
      setAlertConfig({
        ...
        type: 'warning',
        title: 'Validasi',
        message: 'Lokasi awal wajib dipilih',
        buttons: [{ text: 'OK', onPress: () => {} }]
      });
      return false;
    }
  ...
  const handleAddToList = () => {
    ...
    const newAssignment = {
      ...
      equipmentKategori: selectedEquipment?.kategori,
      equipmentModel: selectedEquipment?.model,
      equipmentManufaktur: selectedEquipment?.manufaktur,
    };

    // Add to Redux store
    dispatch(addDraftAssignment(newAssignment));

    console.log('📋 Daftar Penugasan Updated (Redux):', {
      totalPenugasan: draftAssignments.length + 1,
      penugasanBaru: {
        id: newAssignment.id,
        karyawan: newAssignment.labels.karyawan,
        equipment: newAssignment.labels.equipment,
        kategori: newAssignment.equipmentKategori,
        model: newAssignment.equipmentModel,
        shift: newAssignment.labels.shift,
        lokasi: newAssignment.labels.lokasi,
        lokasi_to: newAssignment.labels.lokasi_to || '-',
        kegiatan: newAssignment.labels.kegiatan,
        tanggal: moment(newAssignment.tanggal_tugas).format('DD/MM/YYYY'),
      },
    });

    setFormData({
      tanggal_tugas: formData.tanggal_tugas,
    ...
    setAlertConfig({
      ...
      type: 'success',
      title: 'Sukses',
      message: 'Penugasan berhasil ditambahkan ke daftar',
      buttons: [{ text: 'OK', onPress: () => {} }]
    });
  };

  const handleRemoveFromList = (id) => {
    const removedItem = draftAssignments.find(item => item.id === id);
    
    // Remove from Redux store
    dispatch(removeDraftAssignment(id));
    
    console.log('🗑️ Penugasan Dihapus (Redux):', {
      idDihapus: id,
      penugasanDihapus: {
        karyawan: removedItem?.labels.karyawan,
        equipment: removedItem?.labels.equipment,
        kategori: removedItem?.equipmentKategori,
      },
      sisaPenugasan: draftAssignments.length - 1,
    });
  };
  ...
  const handleSubmit = async () => {
    if (draftAssignments.length === 0) {
      setAlertConfig({
        ...
        type: 'warning',
        title: 'Validasi',
        message: 'Belum ada penugasan yang ditambahkan ke daftar',
        buttons: [{ text: 'OK', onPress: () => {} }]
      });
      return;
    }

    console.log('📤 Mengirim Penugasan ke Server (Bulk):', {
      totalPenugasan: draftAssignments.length,
      tanggal: moment(draftAssignments[0]?.tanggal_tugas).format('DD/MM/YYYY'),
      site_id: draftAssignments[0]?.site_id,
      summary: {
        total: draftAssignments.length,
        DT: draftAssignments.filter(a => a.equipmentKategori === 'DT').length,
        HE: draftAssignments.filter(a => a.equipmentKategori === 'HE').length,
      },
      detailPenugasan: draftAssignments.map((a, idx) => ({
        no: idx + 1,
        karyawan: a.labels.karyawan,
        equipment: a.labels.equipment,
        kategori: a.equipmentKategori,
        model: a.equipmentModel,
        manufaktur: a.equipmentManufaktur,
        shift: a.labels.shift,
        lokasi: a.labels.lokasi,
        lokasi_to: a.labels.lokasi_to || '-',
        kegiatan: a.labels.kegiatan,
        penyewa: a.labels.penyewa,
      })),
      dataYangDikirim: draftAssignments
    });

    try {
      // Clean data before sending (remove UI-only fields and fix empty strings)
      const cleanedData = draftAssignments.map((assignment, index) => {
        const { id, labels, equipmentKategori, equipmentModel, equipmentManufaktur, ...cleanData } = assignment;
        
        // Convert empty strings to null for optional numeric fields
        if (cleanData.lokasi_to === '' || cleanData.lokasi_to === undefined) {
          cleanData.lokasi_to = null;
        }
        
        // Convert string IDs to numbers if needed
        const numericFields = ['site_id', 'penyewa_id', 'shift_id', 'equipment_id', 'karyawan_id', 'lokasi_id', 'lokasi_to', 'kegiatan_id'];
        numericFields.forEach(field => {
          if (cleanData[field] !== null && cleanData[field] !== undefined && cleanData[field] !== '') {
      ...
      console.log('🧹 Cleaned data for backend:', {
        originalCount: draftAssignments.length,
        cleanedCount: cleanedData.length,
        sampleOriginal: draftAssignments[0],
        sampleCleaned: cleanedData[0],
        allCleanedData: cleanedData,
        dataTypes: cleanedData[0] ? Object.keys(cleanedData[0]).reduce((acc, key) => {
          acc[key] = {
            value: cleanedData[0][key],
            type: typeof cleanedData[0][key],
            isNull: cleanedData[0][key] === null
          };
          return acc;
        }, {}) : {}
      });
      ...
      await dispatch(bulkCreateEquipmentPlan(cleanedData)).unwrap();
      
      console.log('✅ Semua Penugasan Berhasil Dikirim (Bulk):', {
        jumlah: draftAssignments.length,
      ...
      setAlertConfig({
        ...
        type: 'success',
        title: 'Sukses',
        message: `${draftAssignments.length} penugasan berhasil dikirim`,
        buttons: [{ text: 'OK', onPress: () => router.back() }]
      });
      
      // Draft assignments will be cleared automatically in Redux after successful submission
    } catch (error) {
      console.error('❌ Gagal Mengirim Penugasan (Bulk):', {
        error: error.toString(),
        message: error.message,
        stack: error.stack,
        jumlahPenugasan: draftAssignments.length
      });
  ...
  return (
    <AppScreen>
      ...
      <View style={styles.container}>
        {/* Detail Penugasan - Fixed at Top */}
        <DetailPenugasanCard 
          assignmentList={draftAssignments}
          onViewList={() => {
            console.log('👁️ Membuka Detail Daftar Penugasan (Redux):', {
              totalPenugasan: draftAssignments.length,
              summary: {
                total: draftAssignments.length,
                DT: draftAssignments.filter(a => a.equipmentKategori === 'DT').length,
              ...
              daftarPenugasan: draftAssignments.map((a, idx) => ({
                no: idx + 1,
                karyawan: a.labels.karyawan,
                equipment: `${a.equipmentKategori} - ${a.labels.equipment}`,
                shift: a.labels.shift,
                lokasi: a.labels.lokasi,
                lokasi_to: a.labels.lokasi_to || '-',
                kegiatan: a.labels.kegiatan,
            ...
            router.push({
              pathname: '/penugasan/list-draft',
              params: { assignments: JSON.stringify(draftAssignments) }
            });
          }}
          isDark={isDark}
        />
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.actionContainer}>
            ...
            >
              <Add size={20} color="#f97316" variant="Bold" />
              <Text style={styles.addButtonText}>Tambah ke Daftar</Text>
            </TouchableOpacity>

            <TouchableOpacity 
              style={[styles.submitButton, draftAssignments.length === 0 && styles.submitButtonDisabled]}
              onPress={handleSubmit}
              disabled={bulkCreateLoading || draftAssignments.length === 0}
            >
              <Text style={styles.submitButtonText}>
                {bulkCreateLoading ? 'Mengirim...' : `Kirim ${draftAssignments.length} Penugasan`}
              </Text>
            </TouchableOpacity>
          </View>
