...
import { Clock, Dislike, Like1, Trash } from 'iconsax-react-native'
...
const ShowApprovalChecklog = () => {
    ...
    const changeStatusKehadiran = async (sts) => {
        ...
        try {
            const resp = await apiFetch.post(`/mobile/check-out-mobile/${params.id}/approve`, {...data, approve_sts: "A", kehadiran_sts: sts})
            if(resp.status === 201){
                route.goBack()
            }else{
                setLoading(false)
                ...
                <ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={onRefreshHandle}/>}>
                    <VStack px={3} flex={1}>
...

                        {
                            !data.approve_at &&
                            <HStack>
                                <Button onPress={rejectHandle} w={"1/4"} mr={2} bg={appcolor.teks[mode][5]}>
                                    <HStack space={1} alignItems={"center"}>
...
export default ShowApprovalChecklog