...
import { VStack, Text, HStack, Button, Modal, Input } from 'native-base'
import { useDispatch, useSelector } from 'react-redux'
import { ArrowRight2, Calendar1, MessageText1, User, Calculator, ProfileTick } from 'iconsax-react-native'
...
const ShowFormCuti = () => {
    ...
    const onGetDataHandle = async () => {
        try {
            const resp = await apiFetch.get(`hrd/permohonan-izin-cuti/${params.id}/show`)
            // console.log(resp);
    ...
    return (
        <AppScreen>
            <VStack h={"full"}>
                <HeaderScreen title={"Update Informasi Cuti"} onBack={true} onThemes={true} onNotification={true}/>
                <VStack px={3} flex={1}>
                    <VStack mt={1}>
                        <HStack 
                            p={2} 
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        <DatePicker
                            ...
                            onConfirm={(date) => {
                                ...
                                setData({...data, cuti_start: date, jumlah: finish.diff(start, 'day') + 1})
                            }}
                            onCancel={() => {
                        ...
                        <DatePicker
                            ...
                            onConfirm={(date) => {
                                ...
                                setData({...data, cuti_end: date, jumlah: finish.diff(start, 'day') + 1})
                            }}
                            onCancel={() => {
