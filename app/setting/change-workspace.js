...
export default function ChangeWorkspaceScreen() {
  ...
  const developmentStages = [
    ...
    { label: 'Design', progress: 80, status: 'in-progress' },
    { label: 'Development', progress: 50, status: 'in-progress' },
    { label: 'Testing', progress: 15, status: 'pending' },
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <VStack space={6} p={4}>
            <Animated.View style={{ opacity: fadeAnim }}>
              <VStack bg={mode === 'dark' ? '#14532d' : '#d1fae5'} p={8} rounded="3xl" alignItems="center" space={4}>
                <Center w={32} h={32} bg="rgba(255,255,255,0.3)" rounded="full" mb={2}>
                  <Text fontSize="6xl">🏢</Text>
                </Center>
                <VStack alignItems="center" space={2}>
                  <Text fontSize="2xl" fontFamily="Quicksand-Bold" color={mode === 'dark' ? '#22c55e' : '#16a34a'} textAlign="center">
                    Segera Hadir!
                  </Text>
                  <Text fontSize="sm" fontFamily="Poppins-Light" color={mode === 'dark' ? '#22c55e' : '#16a34a'} textAlign="center" opacity={0.9}>
                    Fitur Ubah Lingkup Kerja sedang dalam pengembangan
                  </Text>
                </VStack>
                <HStack space={2} mt={2}>
                  {[0, 1, 2].map((i) => (
